...
description: KV 캐시·LLM 추론 최적화 분야의 동향 보고서를 만들 때 사용하는 다단계 검색 파이프라인. 서빙 시스템, KV 양자화, 토큰 축출·희소 어텐션, 분산·분리 서빙, 아키텍처 수준 KV 절감, 장문맥·오프로딩의 6개 서브토픽으로 나눠 R1~R6 검색 → W1 보고서 작성 → I1 인덱스 갱신 순으로 실행합니다. `interests/kv-cache-*` 같은 KV 캐시 관련 명세로 `/collect-trends`가 호출되거나 `research-trend-collector` 에이전트가 KV 캐시 토픽을 감지했을 때 따르세요.
---

KV 캐시 관련 동향 보고서를 만들 때 이 스킬에 정의된 **8단계 순차 파이프라인**을 그대로 따른다. 일반 토픽 명세라면 이 스킬은 적용하지 않는다.
...
각 단계는 `general-purpose` 에이전트를 개별 호출한다. **에이전트 하나 = 검색 주제 하나**. 보고서 작성은 하지 않으며, 찾은 논문 목록(제목·URL·Venue/Year·기여·수치)만 텍스트로 반환한다. 에이전트 결과를 다음 단계로 전달하기 위해 메인 컨텍스트에 누적한다.

R1이 끝난 뒤 R2, R2가 끝난 뒤 R3 순서로 하나씩 실행한다. **동시에 실행하지 않는다.**

**각 R 에이전트 프롬프트에 반드시 포함할 내용**:
- `KNOWN_URLS` 목록을 전달하며 "이 URL들은 이미 수집됨 — 제외하고 새 논문만 반환하세요"라고 명시한다.
- `KNOWN_URLS`가 비어 있으면 해당 지시는 생략한다.
...
R1~R6 결과를 모두 프롬프트에 포함하여 `general-purpose` 에이전트를 한 번 호출한다.
- R1~R6가 반환한 **신규 논문만** 포함한 오늘의 새 보고서를 작성한다. 기존 보고서를 읽거나 병합하지 않는다.
- 신규 논문이 하나도 없으면 파일을 생성하지 않고 "신규 논문 없음"을 반환한다.
...
- 신규 논문 수를 완료 메시지에 포함한다.

## Phase 3: 인덱스 갱신 (I1)

`report-indexer` 서브에이전트(`subagent_type: report-indexer`)를 호출해 README의 `<!-- BEGIN: AUTO-INDEX -->` ~ `<!-- END: AUTO-INDEX -->` 구간을 갱신한다.
...
- R1~R6를 **동시에 실행하지 말고** 반드시 순차적으로 하나씩 실행한다.
- 각 R 에이전트는 **검색만** 수행하고 파일을 저장하지 않는다.
- W1만 보고서 파일을 저장한다.
- 명세 파일을 수정하지 않는다.
